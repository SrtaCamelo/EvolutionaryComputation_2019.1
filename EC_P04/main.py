...
import bfo_evo as bfo

#import neural_network as nn
import random_forest as rf
import pso_evo as pso
import gso_n as gso
#import gso_evo as gso
...
#--------- Call Neural NetWork-------------
    ...
    #accu_ga = ga.ga(population.copy(),x_train, y_train, x_test, y_test,x_validation,y_validation)
    #accu_ee = ee.es(population.copy(),x_train, y_train, x_test, y_test,x_validation,y_validation)
    #accu_de = de.de(population.copy(),x_train, y_train, x_test, y_test,x_validation,y_validation)
    #accu_pe = ep.ep(population.copy(),x_train, y_train, x_test, y_test,x_validation,y_validation)
    #accu_eec = eec.es(population.copy(),x_train, y_train, x_test, y_test,x_validation,y_validation)
    #accu_eec2 = eec2.es(population.copy(),x_train, y_train, x_test, y_test,x_validation,y_validation)
    #accu_gso = gso.gso( 0, 1, 50, population.copy(),x_train, y_train, x_test, y_test,x_validation,y_validation)
    #accu_pso = pso.pso(population.copy(),x_train, y_train, x_test, y_test,x_validation,y_validation)
    accu_bfo = bfo.bfo(population.copy(),x_train, y_train, x_test, y_test,x_validation,y_validation)
    ...
    gso_accu.append(accu_gso)
    """
    #pso_accu.append(accu_pso)
    #gso_accu.append(accu_gso[0])
    bfo_accu.append(accu_bfo)
...
List_ofLists =[(normal_accu, "Normal RF"),(ga_accu,"Genetic Algoritm"),(de_accu,"Discrete Evolution"), (ee_accu,"Evolutionary Estrategy"),(ep_accu,"Evolutionary Programming"),(ee_caucht_accu, "Cauchy EE normal"), (ee_cauchy2_accu, "Cauchy EE Crossover"), (gso_accu, "GSO"), (pso_accu, "PSO")]
